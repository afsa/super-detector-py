...
from src.visualization.visualization_helpers import get_mean_voltage


class IvPlot:

    def __init__(self, input_path: str, output_file: Optional[str] = None,
                 output_format: str = 'pdf',
                 marker_size: int = 5, logger: logging.Logger = None,
    ...
    def show(self):
        ...
        if Path(self.input_path).is_file():
            ...
            files = [path.join(self.input_path, f) for f in
        ...
        for f in tqdm(files):
            ...
            if self.save:
                ...
                if len(files) == 1:
                    ...
                    path_head, path_tail = path.split(f)
                    path_tail_name, path_tail_ext = path.splitext(path_tail)
                    plt.savefig(path.join(self.output_file,
                                          '{}.{}'.format(path_tail_name,
                                                         self.output_format)))
